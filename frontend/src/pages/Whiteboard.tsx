...
const Whiteboard = () => {
  ...
  const [history, setHistory] = useState<Record<string, { stack: Shape[][], step: number }>>({
    "1": { stack: [[]], step: 0 }
  });
      ...
      try {
        ...
        if (res.data && res.data.elements) {
            ...
            if (Array.isArray(data) && data.length > 0 && 'elements' in data[0]) {
                ...
                const initialHistory: Record<string, { stack: Shape[][], step: number }> = {};
  ...
  const updatePageElements = (newElements: Shape[], save = false, emit = false) => {
  ...
  const redo = () => {
    const currentHistory = history[activePageId];
    if (currentHistory && currentHistory.step < currentHistory.stack.length - 1) {
        const newStep = currentHistory.step + 1;
        const newElements = currentHistory.stack[newStep];
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-gray-100">
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-white shadow-md rounded-lg p-2 flex gap-2 z-10 border items-center">
        <Button variant="ghost" size="icon" onClick={prevPage} disabled={pageIndex === 0}>
            <ChevronLeft className="h-4 w-4" />
        </Button>
        <span className="text-sm font-medium">
            Page {pageIndex + 1} / {pages.length}
        </span>
        <Button variant="ghost" size="icon" onClick={nextPage} disabled={pageIndex === pages.length - 1}>
        ...
        <Button variant="ghost" size="icon" onClick={deletePage} disabled={pages.length <= 1} title="Delete Page">
